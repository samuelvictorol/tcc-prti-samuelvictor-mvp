...
describe('acesso publico ao perfil', () => {
  ...
  it('mantem rota publica, menu administrativo e UX sem promessa neutra falsa', () => {
    ...
    expect(profilePage).toContain('Código enviado pelos canais disponíveis.')
    ...
    expect(loginSettings).toContain('verify_code_1')
    expect(loginSettings).toContain("pt_BR")
    expect(loginSettings).toContain('BODY e BUTTON (índice 0)')
    expect(loginSettings).toContain('providers.telegram?.configured')
    expect(loginSettings).toContain('template.prerequisite')
    expect(loginSettings).toContain('templateBodyExample')
    expect(loginSettings).toContain('nunca é presumida manualmente')
  })
