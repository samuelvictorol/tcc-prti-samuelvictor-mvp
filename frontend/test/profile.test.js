...
describe('acesso publico ao perfil', () => {
  ...
  it('mantem rota publica, menu administrativo e UX sem promessa neutra falsa', () => {
    ...
    expect(profilePage).toContain('/gerar-codigo')
    expect(profilePage).toContain('notify_profile_pending_challenge')
    ...
    expect(loginSettings).toContain('/gerar-codigo')
    expect(loginSettings).toContain('janela oficial de atendimento')
    expect(loginSettings).toContain('providers.telegram?.configured')
    expect(loginSettings).toContain('template.prerequisite')
    expect(loginSettings).toContain('Nenhum template de autenticação da Meta')
  })
