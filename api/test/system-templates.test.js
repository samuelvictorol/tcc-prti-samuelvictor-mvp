...
test('verify_code_1 usa uma variavel codigo no BODY e no botao OTP de copiar', () => {
  const definition = listSystemTemplateDefinitions().find((item) => item.externalTemplateName === 'verify_code_1');
  const normalized = templatesManager.normalizeTemplateInput(definition);
  assert.equal(normalized.languageCode, 'pt_BR');
  assert.deepEqual(normalized.variables, ['codigo']);
  assert.deepEqual(normalized.payload.builder.components.map((component) => ({
    type: component.type,
    subType: component.subType,
    keys: component.parameters.map((parameter) => parameter.key),
    parameterTypes: component.parameters.map((parameter) => parameter.type)
  })), [
    { type: 'body', subType: undefined, keys: ['codigo'], parameterTypes: ['text'] },
    { type: 'button', subType: 'otp_copy_code', keys: ['codigo'], parameterTypes: ['text'] }
  ]);
...
test('seed cria somente ausentes e e idempotente sem sobrescrever personalizacoes', async () => {
  ...
  try {
    ...
    assert.deepEqual(await templatesManager.ensureSystemTemplates(), { created: 3, protected: 0, existing: 0 });
    ...
    created.length = 0;
    updates.length = 0;
    assert.deepEqual(await templatesManager.ensureSystemTemplates(), { created: 0, protected: 3, existing: 0 });
